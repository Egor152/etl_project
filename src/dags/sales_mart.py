...
postgres_conn_id = 'скрыл по просьбе Яндекс Практикума'
...
headers = {
    ...
    'X-Project': 'True',
    'X-API-KEY': api_key,
    'Content-Type': 'скрыл по просьбе Яндекс Практикума'
}
...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    s3_filename = f'скрыл по просьбе Яндекс Практикума{cohort}/{nickname}/project/{increment_id}/{filename}'
    print(s3_filename)
...
with DAG(
    ...
    #Загружаю данные в таблицу товаров
    update_d_item_table = PostgresOperator(
        task_id='update_d_item',
        postgres_conn_id=postgres_conn_id,
        sql="sql/mart.d_item.sql")

    #Загружаю данные в таблицу покупателей
    update_d_customer_table = PostgresOperator(
        task_id='update_d_customer',
        postgres_conn_id=postgres_conn_id,
        sql="sql/mart.d_customer.sql")

    #загружаем данные в таблицу городов
    ...
    # вставка данных в таблицу    mart.f_customer_retention
